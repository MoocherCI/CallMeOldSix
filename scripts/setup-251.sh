...
sudo ufw allow proto tcp from ${INTERNAL_SUBNET} to any port 80
sudo ufw allow proto tcp from ${INTERNAL_SUBNET} to any port 443
sudo ufw allow proto tcp from ${INTERNAL_SUBNET} to any port 3100
...
echo "  The certbot package (certbot/python3-certbot-dns-cloudflare)"
echo "  auto-installs a systemd timer (certbot.timer) that runs"
echo "  'certbot renew' twice daily. This ensures certificates are"
echo "  renewed automatically before they expire."
echo ""
echo "  Verify timer status:"
echo "    systemctl list-timers certbot.timer"
echo ""
echo "  Test renewal configuration (dry-run — no actual renewal):"
echo "    certbot renew --dry-run"
echo ""
echo "  Configure deploy-hook so nginx reloads on renewal:"
echo "    certbot renew --deploy-hook \"systemctl reload nginx\""
echo ""
echo "  NOTE: The deploy-hook should be added to certbot's renewal"
echo "        configuration so it runs automatically with the timer."
echo "        You can also add it to /etc/letsencrypt/cli.ini:"
